...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddLine(ImVec2(total_bb.Min.x, grab_bb.Max.y - 8), ImVec2(total_bb.Max.x, grab_bb.Max.y - 8), frame_col, 2.0f);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x) {
        //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        if (g.ActiveId == id) {
            window->DrawList->AddCircleFilled(ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), 8, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrab : ImGuiCol_SliderGrabActive), 30);
        }
        else {
            window->DrawList->AddCircleFilled(ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), 6, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 30);
        }
    }
