...
static bool CheckShader(GLuint address, const char* desc)
{
    GLint status = 0, log_length = 0;
    glGetShaderiv(address, GL_COMPILE_STATUS, &status);
    glGetShaderiv(address, GL_INFO_LOG_LENGTH, &log_length);
    ...
    if (log_length > 1)
    {
        ...
        glGetShaderInfoLog(address, log_length, NULL, (GLchar*)buf.begin());
        fprintf(stderr, "%s\n", buf.begin());
    }
...
static bool CheckProgram(GLuint address, const char* desc)
{
    GLint status = 0, log_length = 0;
    glGetProgramiv(address, GL_LINK_STATUS, &status);
    glGetProgramiv(address, GL_INFO_LOG_LENGTH, &log_length);
    if ((GLboolean)status == GL_FALSE)
    ...
    if (log_length > 1)
    {
        ImVector<char> buf;
        buf.resize((int)(log_length + 1));
        glGetProgramInfoLog(address, log_length, NULL, (GLchar*)buf.begin());
        fprintf(stderr, "%s\n", buf.begin());
    }
    return (GLboolean)status == GL_TRUE;
}
